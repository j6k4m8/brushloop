# BrushLoop Monorepo

BrushLoop is a collaborative drawing app for iOS, Android, web, and desktop. This monorepo contains both the Flutter client and a Node.js backend.

## Repository Layout

-   `app/`: Flutter client (iOS, Android, web, macOS).
-   `server/`: Node.js + TypeScript backend using SQLite and local disk storage.
-   `packages/shared/`: Shared TypeScript domain contracts and protocol definitions.
-   `docs/`: Architecture, deployment, and operational docs.

## Prerequisites

-   Node.js 20+ (tested with Node 25.1.0).
-   Flutter 3.38+.

## Quick Start

1. Install dependencies where needed:
    - Root: `npm install`
    - Flutter app: `cd app && flutter pub get`
2. Start backend:
    - `npm run start -w @brushloop/server`
3. Run Flutter app:
    - `cd app && flutter run`

For compile-time API base URL override in Flutter:

```bash
flutter run --dart-define=BRUSHLOOP_API_BASE_URL=http://127.0.0.1:8787
```

## Docker Deployment

Use Compose to run API + TLS edge proxy with persistent data mounts.

Deployment configuration is in one place:

- `deploy/domains.env`

Deploy (pull prebuilt GHCR images; no Flutter install on host):

```bash
./deploy/up.sh
```

This runs:

- Image pull (`api`, `edge`)
- API + edge startup from pulled images

Container images are published by GitHub Actions (`.github/workflows/publish-containers.yml`).
The web domain serves the Flutter app from the `edge` container image.
Persistent API state is mounted at `./deploy/data`.

## Quality Commands

-   `npm run format`
-   `npm run lint`
-   `npm run test`

## Documentation

-   `docs/architecture.md`
-   `docs/deployment.md`
-   `docs/api.md`
